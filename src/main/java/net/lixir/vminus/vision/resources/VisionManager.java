...
public class VisionManager extends SimpleJsonResourceReloadListener {
    ...
    protected @NotNull Map<ResourceLocation, JsonElement> prepare(ResourceManager resourceManager, @NotNull ProfilerFiller profilerFiller) {
        ...
        for(Map.Entry<ResourceLocation, Resource> entry : resourceManager.listResources(this.directory, (p_215600_) -> p_215600_.getPath().endsWith(".json")).entrySet()) {
            ...
            try {
                ...
                VMinus.LOGGER.error("Couldn't parse data file {} from {}", resourcelocation1, resourcelocation, jsonParseException);
            }
        }
    ...
    protected void apply(
        ...
        for (Map.Entry<ResourceLocation, JsonElement> jsonFile : resourceLocationJsonElementMap.entrySet()) {
            try {
                JsonObject jsonObject = VisionResourceProcessor.processJsonObject(this.directory, jsonFile.getValue());
                ...
                for (String key : jsonObject.keySet()) {
                    ...
                    if (mainJsonObject.has(key)) {
                        ...
                        VisionResourceProcessor.mergeJsonObjects(existingObject, newObject, 0);
                    } else {
                        mainJsonObject.add(key, value);
                    }
                }
            } catch (Exception e) {
                VMinus.LOGGER.error("Error processing JSON file: {}", jsonFile.getKey(), e);
            }
        }
            ...
            if (jsonFiles != null) {
                for (File file : jsonFiles) {
                    try (FileReader reader = new FileReader(file)) {
                        ...
                        try {
                            JsonObject jsonObject = VisionResourceProcessor.processJsonObject(this.directory, configJson);
                            ...
                            VMinus.LOGGER.error("Error processing JSON file: {}", file.getName(), e);
                        }
                    } catch (IOException e) {
                        VMinus.LOGGER.error("Error reading config: {}", file.getName(), e);
                    }
                }
            }
        }
