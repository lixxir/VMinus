package net.lixir.vminus.api.datagen.sound.provider;
...
public abstract class VSoundDefinitionProvider extends SoundDefinitionsProvider {
    ...
    protected SoundDefinition generateSoundDefinition(@NotNull SoundDefinitionInfo info) {
        ...
        if (paths != null && !paths.isEmpty()) {
            ...
            for (int i = 0; i < paths.size(); i++) {
                sounds[i] = sound(parseResourceLocation(paths.get(i)));
            }
            ...
            if (count == 1) {
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    SoundDefinition.Sound s = sound(parseResourceLocation(fullFilePath));
                    sounds[i] = s;
    ...
    protected ResourceLocation parseResourceLocation(@NotNull String path) {
        if (path.contains(":")) {
