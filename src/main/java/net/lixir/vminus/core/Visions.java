...
public class Visions {
    ...
    private static JsonObject scanVisionJsonKey(JsonObject mainVision, String key, String id, JsonObject mergedData,
        ...
        for (String matchKey : parts) {
           ...
           if (matchKey.equals(id) || matchKey.equals("global") || matchKey.equals("all")) {
                found = true;
            } else if (isTag) {
    ...
    public static @Nullable JsonObject getData(@Nullable Object target) {
        ...
        } else if (target instanceof BlockBehaviour.BlockStateBase _base) {
            return getData(_base.getBlock());
    ...
    public static @Nullable JsonObject getData(@Nullable Player player) {
        return getData(player, false, -1);
    }

    public static @Nullable JsonObject getData(@Nullable Player player, @Nullable Boolean debug, int key) {
        if (player == null)
            return null;
        String id = attemptGetObjectId(player);
        if (id.isEmpty())
            return null;
        VisionType visionType = getVisionType(player);
        if (visionType == null)
            return null;
        return processVisionData(visionType, id, player, key);
    }
    ...
    private static String attemptGetObjectId(@Nullable Player player) {
        if (player == null) {
            return null;
        }
        return Objects.requireNonNull(ForgeRegistries.ENTITY_TYPES.getKey(player.getType())).toString();
    }
    ...
    private static @Nullable VisionType getVisionType(@Nullable Player player) {
        if (player == null) {
            return null;
        }
